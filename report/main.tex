...
The current repository now provides a runnable experimental package at \texttt{src/geo\_uav\_recon}. The implementation has already been updated from the early single-dataset scaffold into a two-dataset benchmark centered on \texttt{Dronescapes + ODMData}:
...
\midrule
\texttt{dataset.py} & discovers \texttt{Dronescapes} and \texttt{ODMData} layouts and normalizes image, pose, and optional depth inputs \\
\texttt{realdata.py} & prepares official dataset downloads, validates local copies, and writes benchmark-ready manifests \\
\texttt{predictors.py} & wraps external baselines such as \texttt{DUSt3R}, \texttt{MASt3R}, and \texttt{COLMAP + OpenMVS} \\
\texttt{risk.py} & computes reconstruction risk from overlap, texture, depth gap, and viewing diversity \\
\texttt{benchmark.py} & orchestrates evaluation, pseudo-reference handling for \texttt{ODMData}, and report export \\
\texttt{cli.py} & exposes dataset preparation, validation, export, and benchmark commands \\
\bottomrule
\end{tabularx}
\caption{Implemented package for the current GEO benchmark direction.}
\end{table}

The current code is not a final publishable method. It is a \textbf{working experimental package} that is already sufficient to:
\begin{enumerate}[leftmargin=1.5em]
  \item prepare and validate the two real datasets used in the project,
  \item run classical and learned reconstruction baselines end to end,
  \item rank frames by reconstruction risk,
  \item and benchmark selective refinement against full-pass alternatives.
\end{enumerate}
